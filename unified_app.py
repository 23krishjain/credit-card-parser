...
class UnifiedCreditCardApp:
    ...
    def get_bank_style(self, bank_name):
        bank_classes = {
            ...
            'Axis': 'axis', 'Kotak': 'kotak', 'Chase': 'chase',
            'Amex': 'amex', 'Citi': 'citi', 'Discover': 'discover',
            'BankOfAmerica': 'bofa'
        }
    ...
    def display_transactions(self, statement):
        ...
        st.dataframe(df, use_container_width=True, height=min(400, len(transactions_data) * 35 + 38))
    ...
    def run(self):
        ...
        **Features:**
        ...
        uploaded_file = st.file_uploader("Choose a PDF file", type="pdf", help="Upload your credit card statement in PDF format")

        if uploaded_file is not None:
            st.write({"Filename": uploaded_file.name, "File size": f"{uploaded_file.size / 1024:.2f} KB"})
            col1, col2 = st.columns(2)
            with col1:
                use_advanced = st.checkbox("Use advanced table extraction", value=True, help="Use Camelot/Tabula for better table detection")
            with col2:
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                st.markdown("#### 1. Upload"); st.write("Upload your credit card statement PDF")
            with col2:
                st.markdown("#### 2. Parse"); st.write("Automatic detection and extraction")
            with col3:
                st.markdown("#### 3. Analyze"); st.write("View transactions and download results")
            st.markdown("---")
